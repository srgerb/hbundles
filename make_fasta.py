...
def get_aa_sequence_from_pdb(pdb_file):
    ...
    for i in range(0, len(pdb)):
        if (pdb[i] != None and pdb[i][0] == "ATOM"):
...
parser.add_argument('-o','--out', type=str, nargs=1, help="Name of output file")
...
for i in range(0, len(input_file) - 4):# removes .pdb from end of file name
    ...
    if input_file[i] == "/": #removes path before file name
       tag = ""
if args.out != None:
    output_file_name = args.out[0] + "_" + tag + ".pdb" #adds the filtered chain name to filename
else:

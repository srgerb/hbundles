...
columns = 29 #the number of columns your score file should have 
mismatch = [2,3,4,5,6] #the column that should be deleted
...
for line in range(0, len(output)):
    for column in range(0, len(output[line])):
        if len(output[line]) == columns:
            full_pdb.write(output[line][column] + "\t")
        elif column not in mismatch:
            full_pdb.write(output[line][column] + "\t")
    full_pdb.write("\n")

...
# w1 = 1080./11.
 w1 = 1800./18. #omega for straight helix
 d=1.51
 z1=0.
 z2=0.
...
#######this will place the middle of helix to xy plane
 ...
 stub_file=map(string.split,open('/home/srgerb/scripts/parametric_backbones/ideal.pdb','r').readlines())
 atom=[]
...
def input_params(input):
    ...
    P1l = []
    Zl = []
    #num_to_output is the number of helices
    ...
    num_vars=11
    for i in range(num_to_output):
        offset=num_vars*(i)+4 #allows you to read the input for multiple chains (num_vars is the number of variables we're reading in for each chain, 4 is the number of variables read in before individual chain information)
        Nres.append(int(input[0+offset][0])) #total number of residues in the helix
        Nres_1.append(int(input[1+offset][0])) #number of residues in the first segment of the helix
        num_link_res.append(int(input[2+offset][0])) #number of residues in linker between helix segments
        w0l_1.append( (float(input[3+offset][0]),float(input[3+offset][1]),int(input[3+offset][2]))) #first seg supercoil twist
        w0l_2.append( (float(input[4+offset][0]),float(input[4+offset][1]),int(input[4+offset][2]))) #second seg supercoil twist
        R0l_1.append( (float(input[5+offset][0]),float(input[5+offset][1]),int(input[5+offset][2]))) #supercoil radius of first segment
        R0l_2.append( (float(input[6+offset][0]),float(input[6+offset][1]),int(input[6+offset][2]))) #supercoil radius of second segment
        P0l.append( (float(input[7+offset][0]),float(input[7+offset][1]),int(input[7+offset][2]))) #supercoil phase
        orientation.append( int(input[8+offset][0] )) # orientation of helix
        P1l.append( (float(input[9+offset][0]),float(input[9+offset][1]),int(input[9+offset][2]))) # helix phase of first segment
        Zl.append( (float(input[10+offset][0]),float(input[10+offset][1]),int(input[10+offset][2])))  # z-axis offset
...
input_file=argv[1]
tag = argv[2]
#input takes input_file and makes a map. this takes the form of a 2D array of strings[line number][string on line]
...
for combo in combos:
	...
	if i==0 :
		if w0_2[i][0]=="constrained" and R0_2[i][0]=="constrained":
		    w0v_2.append(w0_1[0][id[0]])
		    R0v_2.append(R0_1[0][id[2]])
		elif w0_2[i][0]=="constrained" and R0_2[i][0]!="constrained":
	    	    w0v_2.append( w0_1[0][id[0]]/math.sqrt(1+(math.pow(R0_2[i][id[3+i*7]],2)-math.pow(R0_1[0][id[2]],2))*math.pow(w0_1[0][id[0]]*math.pi/180,2)/math.pow(1.51,2)))
            	    R0v_2.append(R0_2[0][id[3]])
		elif w0_2[i][0]!="constrained" and R0_2[i][0]=="constrained":
		    print "ERROR!! Twist might be constrained if radius is constrained"
		    sys.exit()
		else:
	        ...
	        if len(w0_2[i])==1 and w0_2[i][0]=="constrained" and len(R0_2[i])==1 and R0_2[i][0]=="constrained":
        	    w0v_2.append(w0v_2[0])
		    R0v_2.append(R0v_2[0])
       	        elif len(w0_2[i])==1 and w0_2[i][0]=="constrained" and R0_2[i][0]!="constrained":
		    w0v_2.append( w0v_2[0]/math.sqrt(1+(math.pow(R0_2[i][id[3+i*7]],2)-math.pow(R0v_2[0],2))*math.pow(w0v_2[0]*math.pi/180,2)/math.pow(1.51,2)))
		    R0v_2.append(R0_2[i][id[3+i*7]])
       	        elif len(R0_2[i])==1 and R0_2[i][0]=="constrained" and w0_2[i][0]!="constrained":
		    print "ERROR!! Twist might be constrained if radius is constrained"
                    sys.exit()
	        else:
        	    w0v_2.append(w0_2[i][id[1+i*7]])

...
for i in range(0, last_chain_number):
    ...
    cstfile.write("AtomPair VNE {resnumber}{chain} VX2 {resnumber}{next_chain} HARMONIC 0.0 0.3 \n".format(resnumber = HID_resnum, chain = chains[i], next_chain = chains[(i+1)%last_chain_number] ))
    cstfile.write("AtomPair VNE {resnumber}{chain} VX3 {resnumber}{next_chain} HARMONIC 0.0 0.3 \n".format(resnumber = HID_resnum, chain = chains[i], next_chain = chains[(i+1)%last_chain_number] ))
    cstfile.write("AtomPair VNE {resnumber}{chain} VX4 {resnumber}{next_chain} HARMONIC 0.0 0.3 \n".format(resnumber = HID_resnum, chain = chains[i], next_chain = chains[(i+1)%last_chain_number] ))
    cstfile.write("END\n\n")
